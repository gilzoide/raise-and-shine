...
func _gui_input(event: InputEvent) -> void:
	if event is InputEventMouseMotion:
		var scaled_rect = get_drawn_rect()
		if scaled_rect.rect.has_point(event.position):
			last_position = ((event.position - scaled_rect.rect.position) / scaled_rect.scale).floor()
...
func get_drawn_rect() -> ScaledRect:
	...
	return ScaledRect.new(rect, scale)
