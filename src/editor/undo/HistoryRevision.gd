...
func set_revision(revision) -> void:
	...
	if revision.selection.is_empty():
		return
	...
	texture_rect.material.set_shader_param("selection_texture_pixel_size", Vector2(1.0 / max(1.0, selection_size.x), 1.0 / max(1.0, selection_size.y)))
